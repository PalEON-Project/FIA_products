...
biomass_taxon <- foreach(taxonIdx = seq_along(taxa_to_fit)) %dopar% {
    ...
    try(fit(sub, newdata = pred_grid_paleon, k_occ = k_occ_taxon_biomass, k_pot = k_pot_taxon_biomass,
            return_model = TRUE, unc = TRUE, type_pot = fit_scale_biomass, num_draws = n_stat_samples,
