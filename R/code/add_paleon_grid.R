...
add_paleon_grid <- function(data, conversions_data_dir, fia_plot_to_paleon_gridcell_file, states, base_raster) {
    ...
    if(!file.exists(file)) {
        ...
        grid <- read_csv(file.path(conversions_data_dir, fia_plot_to_paleon_gridcell_file), 
                         col_types = 'dddcddd') %>% dplyr::select(-LON_ALBERS, -LAT_ALBERS) %>% rename(x = cell_x, y = cell_y)
...
add_grid_to_fia <- function(file, base_raster) {
    ...
    x2 <- read_excel(file.path(conversions_data_dir, nrs_coordinates_file_run2)) %>% select(STATECD, CN, LAT_ACTUAL_NAD83, LON_ACTUAL_NAD83)
    x3 <- read_excel(file.path(conversions_data_dir, nrs_coordinates_file_run3)) %>% select(STATECD, CN, LAT_ACTUAL_NAD83, LON_ACTUAL_NAD83)

    x <- rbind(x1, x2, x3)

    x <- SpatialPointsDataFrame(coords=data.frame(x=x$LON_ACTUAL_NAD83, y=x$LAT_ACTUAL_NAD83),data=data.frame(CN=x$CN, STATECD=x$STATECD), proj4string=CRS("+init=epsg:4269"))
