...
# recognizing that y tells which value is the max, so don't need to scan
# not needed as I can do this quickly in R with precomputation of which column to go to for each row
cppFunction('
 NumericVector rowmax3_cpp(NumericMatrix x, IntegerVector inds, IntegerVector y){
    int n = inds.size();
    int j;
    NumericVector maxvals(n);
    for(j = 0; j < n; j++) 
      maxvals(j) = x(inds[j]-1, y[j]-1);
   return maxvals;
}')
